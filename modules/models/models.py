...
def get_model(
    ...
    try:
        if model_type == ModelType.OpenAIVision or model_type == ModelType.OpenAI:
            ...
            logging.info(f"正在加载DeepSeek模型: {model_name}")
            from .DeepSeek import DeepSeek_Client
            access_key = os.environ.get("DEEPSEEK_API_KEY", access_key)
            api_host = os.environ.get("DEEPSEEK_API_HOST", None)
            model = DeepSeek_Client(model_name, access_key, user_name=user_name)
            if api_host:
                model.api_host = api_host
                model.chat_completion_url = f"https://{api_host}/chat/completions"
                model.base_url = f"https://{api_host}"
                model._refresh_header()
